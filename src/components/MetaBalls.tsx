...
const MetaBalls: React.FC<MetaBallsProps> = ({
  ...
  useEffect(() => {
    ...
    function onPointerMove(e: PointerEvent) {
      ...
      const normalizedX = Math.max(0, Math.min(1, px / rect.width));
      const normalizedY = Math.max(0, Math.min(1, py / rect.height));
      
      pointerX = normalizedX * gl.canvas.width;
      pointerY = (1 - normalizedY) * gl.canvas.height;
      
      // Pass normalized coordinates to parent
      if (onMousePositionRef.current) {
        onMousePositionRef.current(normalizedX, normalizedY);
      }
    }
    ...
    function update(t: number) {
      ...
      if (pointerInside && isFinite(pointerX) && isFinite(pointerY)) {
        targetX = pointerX;
        targetY = pointerY;
      } else {
      ...
      // Add bounds checking and stability
      const smoothing = Math.min(hoverSmoothness, 0.1);
      ...
      mouseBallPos.x = Math.max(0, Math.min(gl.canvas.width, mouseBallPos.x || 0));
      mouseBallPos.y = Math.max(0, Math.min(gl.canvas.height, mouseBallPos.y || 0));
